    ...
    return ( <
        ...
        div className = { classes.paperNavgation } > {
            props.paper.questions.map((questionAnswerObj, questionIndex) => {
                ...
                userAnswer = { props.paper.answers[questionIndex] }
                userFlag = { props.paper.flag[questionIndex] }
                key = { questionIndex }
                questionIndex = { questionIndex }
                currentQuestionIndex = { props.paper.currentQuestionIndex }
                />;
            })
        } <
        /div> <
        /div> <
        form action = { baseUrl + '/submitPaper' }
        method = "POST"
        id = "submitForm" >
        <
        input type = "text"
        ...
        input type = "text"
        name = "answer"
        value = { JSON.stringify(props.paper.answers) }
        hidden / >
        <
        Button type = "primary"
        icon = { < LeftOutlined / > }
        size = "small"
        onClick = { props.backToPaper } > Back < /Button> <
        button type = "submit"
        icon = { < SaveOutlined / > }
