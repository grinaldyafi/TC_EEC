...
const questionnavigation = (props) => {
        ...
        if (props.userAnswer === undefined) { color = '#eee' } else if (props.userAnswer !== "" && props.userAnswer !== null) { color = '#9a9c95' }

        return ( <
            div onClick = { props.click }
            style = {
                { backgroundColor: color, border: borderColor }
            }
            className = { classes.boxes } >
            <
            div className = { classes.boxesNumber } > {
                flag === 'Y' ? < div className = { classes.flag } > < /div> : null} <
                span className = { classes.questionindex } > { props.questionIndex + 1 } <
                /span> < /
                div >
